...
<template>
    <header class="flex justify-between py-4 px-6 w-full">
        <RouterLink to="/" class="justify-center items-center hidden md:flex">
            <img :src="logo" alt="" class="h-10 ">
        </RouterLink>
        <nav class="flex justify-center items-center">
            <ul class="flex md:gap-4 lg:gap-8 gap-8 w-full text-primary font-bold ">
                <RouterLink :to="link.route" v-for="link in landing.links">
                    {{ link.label }}
                </RouterLink>
            </ul>
        </nav>
        <div class="gap-2 justify-center items-center font-bold flex " v-if="!auth.isLogged">
